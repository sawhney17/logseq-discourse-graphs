...
export async function updateDiscourceCSS(e?: any) {
  ...
  const collection: any = parent.document.querySelectorAll(
    "span.page-reference[data-ref]"
  );
  console.log("DB collectionyooo", collection);
...
export async function colorpage() {
  ...
  for (let i = 0; i < collection.length; i++) {
    ...
    if (type) {
      const elements: any = parent.document.querySelector(
        `[data-ref="${page?.name}"]`
      );
